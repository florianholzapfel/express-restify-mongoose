...
module.exports = function (model, excludedKeys, lean) {
    ...
    function filterPojo(pojo, customExclude) {
        if (!pojo) { return; }

        var excluded = customExclude || excludedKeys;
        var excludedarr = excluded.split(',');

        for (var k = 0; k < excludedarr.length; ++k) {
            if (excludedarr[k].indexOf('.') > 0) {
                var pieces = excludedarr[k].split('.'),
                    key = pieces[0],
                    rest = pieces.splice(1).join('.');

                pojo[key] = filterItems(pojo[key], filterPojo, rest);
            } else {
                delete (pojo[excludedarr[k]]);
            }
        }
