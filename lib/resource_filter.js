...
function Filter (opts) {
  ...
  if (this.model.discriminators && _.isPlainObject(opts.excludedMap)) {
...
Filter.prototype.getExcluded = function (opts) {
  ...
  var entry = opts.excludedMap ? opts.excludedMap[opts.modelName] : null
 ...
 */
Filter.prototype.getModelAtPath = function (fullField) {
  var fieldArray = fullField.split('.')
  var currSchema = this.model.schema
  var modelName
  var schemaPath = ''
  ...
  if (!path && !this.model.discriminators) {
    return
  }
...
Filter.prototype.filterPopulatedItem = function (item, opts) {
  ...
  for (var i = 0; i < opts.populate.length; i++) {
    ...
    excluded = self.getExcluded({
      ...
      modelName: self.getModelAtPath(opts.populate[i].path)
    })
