...
const restify = function (app, model, opts) {
  ...
  const filterAndFindById = require("./middleware/filterAndFindById")(
    model,
    options
  );
  ...
  const prepareOutput = require("./middleware/prepareOutput")(
    options,
    excludedMap
  );
  ...
  app.delete(
    ...
    prepareQuery,
    options.preMiddleware,
    options.findOneAndRemove ? [] : filterAndFindById,
    options.preDelete,
    ops.deleteItem,
    prepareOutput
  );
